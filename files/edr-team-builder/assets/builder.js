...
const DRIVER_NOTES={
  'Matthew Halden':'team organiser; still owes Blanchimont an apology',
  'Chris Wilson':'co-leader; steady when it matters',
  'Aden Lennox-Bradley':'the quick one; sarcasm is load-bearing',
  'Jake Lennox-Bradley':'asks the question everyone else was thinking',
  'Matt Blee':'social engine of the team; badly undersells his own pace',
  'Dominic Bou-Samra':'meme department; posts his own crashes first',
  'Laurent Masson':'apologises more than the stewards do',
  'Thomaz Hernandes':'turns up and immediately makes things work',
  'Erik van der Bijl':'the coach. "Do less."',
  'Joseph Tavora':'fast, and the first to prosecute himself for it',
  'Michael Cullen':'hand-writes the guides the rest of us read',
  'Aaron Werth':'loudest and keenest, wins or not',
  'Sam Millar':'IMSA and GT3 regular; races more than anyone',
  'Janne Salminen':'fuel and setup wizard; quietly the fastest',
  'Brock Hellmech':'happy to learn in public, which is rarer than it sounds',
  'Luke Hay':'few words; pays for the liveries',
  'John Nyhouse':'boundless keenness, zero cynicism',
  'Fred Zufelt':'resident F1 analyst; will defend Stroll to anyone',
  ...
  'Bernardo Hickmann':'GT3 regular',
  'Ben Hagstrom':'always in something',
  'Thomas McEwan':'turns up for the odd one out'
};
...
/* An earned driver mention for a round: whoever among the runners has actually raced this track,
   with their own record, so the line changes as the season does. Falls back to the hand-written
   personality note, then to nothing. `used` keeps one driver from being the flare twice. */
function flareFor(who, used, track){
  const w=weeklyState(); if(!w.flare) return null;
  if(haveHistory() && track){
    const t=HISTORY.tracks[histTrackKey(track)];
    if(t && t.drivers && t.drivers.length){
      const cand=t.drivers.filter(function(d){ return (who||[]).indexOf(d.name)>=0 && !(used&&used[d.name]); });
      if(cand.length){
        const d=cand[0], pos=!!(HISTORY.fields && HISTORY.fields.pos);
        if(used) used[d.name]=1;
        let note=d.races+' start'+(d.races===1?'':'s')+' here';
...
function weeklyFacts(){
  ...
  rounds.forEach(function(r){
    ...
    sprints.push({
      ...
      matched:!!r.track, note:trackNoteFor(r.track), flare:flareFor(who, usedDrivers, r.track), hist:histTrackLine(r.track),

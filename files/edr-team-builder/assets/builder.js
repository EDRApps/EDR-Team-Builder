...
function wkChunks(text){
  ...
  t.split(/\n{2,}/).forEach(function(p){
    ...
    if(cur!=='' && wkBytes(cur)+wkBytes(p)+2>WK_LIMIT){ out.push(cur); cur=''; }
    cur+=(cur===''?'':'\n\n')+p;
  });
...
function weeklyDraft(f){
  /* Markdown, because the draft is posted straight into Discord — which renders #/##
     ...
     House layout, applied to every section so the whole post scans the same way:
       ## heading   one per section, carrying the only emoji in the body
       **Name**     the subject of an entry, alone on its line
       plain line   the facts that decide whether you care (track, car, length, when)
       -# subtext   the quiet supporting stuff (history, session times, warnings, flare)
       > quote      the human note
       blank line   between every entry — it is what gives Discord the spacing, and it is
                    also where the 1900-char chunker prefers to split
     Emoji are deliberately rationed to the headings and the top-three medals. Sprinkled
     through the body they stop reading as signal and the write-up looks like a newsletter. */
  const L=[];
...
function wkTidy(L){
  ...
  L.forEach(function(ln){
    ...
    if(s===''){ if(out.length && out[out.length-1]!=='') out.push(''); return; }
    if(/^#{1,3}\s/.test(s) && out.length && out[out.length-1]!=='') out.push('');   // -# is not a heading
    out.push(s);
  });

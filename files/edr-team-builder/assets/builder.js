...
function applyIrWeather(ev, se){  // map an iRacing schedule weather{} onto state.evWeather (F7 auto-pull)
  ...
  state.evWeather[evKey(ev)]={ src:'iracing', startTod:(startTod!=null?startTod:720), mult:(+w.simulated_time_multiplier)||1, tempC:temp, wet:!!wetBase };
}
...
function weatherAt(absMin, raceStartOff){
  ...
  const todMin=((((+w.startTod||0) + elapsed*mult)%1440)+1440)%1440;
